...
export default function AlertSummaryCards({ summary }) {
  if (!summary) return null;
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
      <Stat label="Total"    value={summary.total}    icon={Bell}          tone="indigo" />
      <Stat label="Critical" value={summary.critical} icon={AlertCircle}   tone="red" />
      <Stat label="Warning"  value={summary.warning}  icon={AlertTriangle} tone="amber" />
      <Stat label="Open"     value={summary.open}     icon={Info}          tone="sky" />
    </div>
  );
}